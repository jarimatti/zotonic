        ...
        <tr class="{% if not m.rsc[id].is_published %}unpublished{% endif %}" data-href="{% url admin_edit_rsc id=id %}">
            ...
            <td>
                ...
                <span class="pull-right">
                    {% if id.page_url %}<button href="{{ id.page_url }}" class="btn btn-default btn-xs">{_ view _}</button>{% endif %}
                    <button href="{% url admin_edit_rsc id=id %}" class="btn btn-default btn-xs">{_ edit _}</button>
                </span>
            </td>
        </tr>
        {% endif %}
